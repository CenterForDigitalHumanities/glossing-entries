 ...
 */
class BibliographicCitationDiv extends HTMLElement {
  template = `
        <style>
            .bib-citation-modal {
                display: none; 
    ...
    createCitationCard(citation) {
        const previewContent = citation.citation.length > 40
            ? citation.citation.substring(0, 40) + '…'
            : citation.citation
...
let __constants = await fetch("../properties.json").then(r=>r.json()).catch(e=>{return {}})
...
async function updateBibliographicCitation(citationContent, glossId, citationId) {
    try {
        deleteBibliographicCitation(citationId).then(() => {addBibliographicCitationToGloss(citationContent, glossId)})
    } catch (error) {
...
customElements.define("bibliographic-citation-div", BibliographicCitationDiv)
