...
async function removeFromCollectionAndDelete(event, type, id = null) {
    ...
    // This won't do    
    if (id === null) {
        globalFeedbackBlip(event, `No URI supplied for delete. Cannot delete.`, false);
        return;
    }

    // If it is an unexpected type, we probably shouldn't go through with the delete.
    if (thing === null) {
        globalFeedbackBlip(event, `Not sure what a ${type} is. Cannot delete.`, false);
        return;
    }
    ...
    if (type === "manuscript") {
        ...
        const allGlossIds = await getPagedQuery(100, 0, allGlossesOfManuscriptQueryObj)
            ...
            .catch(err => {
                globalFeedbackBlip(event, "Could not gather Glosses to delete.", false);
                console.log(err);
                return null;
            });
    ...
    const allAnnotationIds = await getPagedQuery(100, 0, allAnnotationsTargetingEntityQueryObj)
        ...
        .catch(err => {
            globalFeedbackBlip(event, "Could not gather Annotations to delete.", false);
            console.log(err);
            return null;
        });
    // This is bad enough to stop here, we will not continue on towards deleting the entity.
    ...
    fetch(`${__constants.tiny}/delete`, {
        ...
        .catch(err => {
            globalFeedbackBlip(event, `There was an issue removing the ${thing} with URI ${id}.  This item may still appear in collections.`, false);
            console.log(err);
        });
}
