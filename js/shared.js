...
/**
 * Delete a Manuscript Witness through manuscript-metadata.html or manage-manuscripts.html.
 * This will delete the Manuscript Witness entity itself, its fragments, and its Annotations.
 */
async function deleteManuscriptWitness(manuscriptWitnessID, redirect) {
    if (!manuscriptWitnessID) return;
    
    // Delete all Witness Fragments that have a "partOf" annotation pointing to the Manuscript Witness ID
    const fragmentAnnosQuery = {
        "body.partOf.value": httpsIdArray(manuscriptWitnessID),
        "__rerum.generatedBy": httpsIdArray(__constants.generator)
    };

    const fragmentAnnos = await getPagedQuery(100, 0, fragmentAnnosQuery)
        .then(annos => annos.map(anno => anno["@id"]))
        .catch(err => {
            console.error(err);
            return [];
    ...
    const deleteFragmentCalls = fragmentAnnos.map(annoUri => {
        return fetch(`${__constants.tiny}/delete`, {
            method: "DELETE",
            body: JSON.stringify({ "@id": annoUri.replace(/^https?:/, 'https:') }),
            headers: {
        ...
        .catch(err => {
            console.warn(`There was an issue removing a Witness Fragment: ${annoUri}`);
            console.log(err);
        });
    });

    // Delete the Manuscript Witness entity itself
    const deleteWitnessCall = fetch(`${__constants.tiny}/delete`, {
        method: "DELETE",
        body: JSON.stringify({ "@id": manuscriptWitnessID.replace(/^https?:/, 'https:') }),
        headers: {
            "Content-Type": "application/json; charset=utf-8",
            "Authorization": `Bearer ${window.GOG_USER.authorization}`
        },
    ...
    .catch(err => {
        console.warn(`There was an issue removing the Manuscript Witness: ${manuscriptWitnessID}`);
        console.log(err);
    });

    if (redirect) {
        Promise.all([...deleteFragmentCalls, deleteWitnessCall])
            .then(success => {
                addEventListener("globalFeedbackFinished", ev => {
                    startOver();
                });
            ...
            .catch(err => {
                console.error("There was an issue removing Witness Fragments or Manuscript Witness.");
                console.error(err);
                const ev = new CustomEvent("Error Deleting Manuscript Witness");
                globalFeedbackBlip(ev, `Error Deleting Manuscript Witness. It may still appear.`, false);
            });
    } else {
        return [...deleteFragmentCalls, deleteWitnessCall];
    }
}
