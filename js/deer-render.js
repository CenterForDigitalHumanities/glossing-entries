...
RENDER.element = function (elem, obj) {
  return UTILS.expand(obj).then((obj) => {
    let tmplName =
      elem.getAttribute(DEER.TEMPLATE) ??
      (elem.getAttribute(DEER.COLLECTION) ? "list" : "json");
    ...
    elem.innerHTML =
      typeof templateResponse.html === "string"
     ...
     */
    setTimeout(function () {
      let newViews = elem.querySelectorAll(config.VIEW).length
        ? elem.querySelectorAll(config.VIEW)
        : [];
      let newForms = elem.querySelectorAll(config.FORM).length
        ? elem.querySelectorAll(config.FORM)
        : [];
      if (newForms.length) {
        UTILS.broadcast(undefined, DEER.EVENTS.NEW_FORM, elem, {
          set: newForms,
        });
      }
      if (newViews.length) {
        UTILS.broadcast(undefined, DEER.EVENTS.NEW_VIEW, elem, {
          set: newViews,
        });
      }
...
DEER.TEMPLATES.prop = function (obj, options = {}) {
  ...
  try {
    ...
    return `<span class="${prop}">${
      UTILS.getValue(prop) ?? "[ no value ]"
    }</span>`;
  } catch (err) {
    return null;
  }
...
DEER.TEMPLATES.linky = function (obj, options = {}) {
  try {
    ...
    return link
      ? `<a href="${UTILS.getValue(
          link
        )}" title="Open in a new window" target="_blank"><img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAYAAACNMs+9AAAAQElEQVR42qXKwQkAIAxDUUdxtO6/RBQkQZvSi8I/pL4BoGw/XPkh4XigPmsUgh0626AjRsgxHTkUThsG2T/sIlzdTsp52kSS1wAAAABJRU5ErkJggg=="></a>`
      : ``;
  } catch (err) {
    return null;
  }
...
DEER.TEMPLATES.thumbs = function (obj, options = {}) {
  return {
    html: obj["tpen://base-project"]
      ? `<div class="is-full-width"> <h3> ... loading images ... </h3> </div>`
      : ``,
    then: (elem) => {
      fetch(
        "https://t-pen.org/TPEN/manifest/" + obj["tpen://base-project"].value
      )
        .then((response) => response.json())
        .then(
          (ms) =>
            (elem.innerHTML = `
                ${ms.sequences[0].canvases
                  .slice(0, 10)
                  .reduce(
                    (a, b) =>
                      (a += `<img class="thumbnail" src="${b.images[0].resource[
                        "@id"
                      ]?.replace("full/full", "full/,120")}">`),
                    ``
                  )}
        `)
        );
...
DEER.TEMPLATES.pageLinks = function (obj, options = {}) {
  ...
  return obj.sequences[0].canvases.reduce(
    (a, b, i) =>
      (a += `<a class="button" href="?page=${i + 1}#${obj["@id"]}">${
        b.label
      }</a>`),
    ``
  );
...
DEER.TEMPLATES.folioTranscription = function (obj, options = {}) {
  // if(!userHasRole(["glossing_user_manager", "glossing_user_contributor", "glossing_user_public"])) { return `<h4 class="text-error">This function is limited to registered Gallery of Glosses users.</h4>` }
  return {
    html: obj.tpenProject
      ? `<div class="is-full-width"> <h3> ... loading preview ... </h3> </div>`
      : ``,
    then: (elem) => {
      ...
      fetch("https://t-pen.org/TPEN/manifest/" + url)
        ...
        .then(
          (ms) =>
            (elem.innerHTML = `
                ${ms.sequences[0].canvases.slice(0, 10).reduce(
                  (a, b) =>
                    (a += `
                <div class="page">
                    <h3>${b.label}</h3> 
                    <a href="./layout.html?partOf=${elem.getAttribute(
                      "deer-partof"
                    )}#${ms["@id"]}">(edit layout)</a>
                    <a href="./align-glosses.html#${
                      ms["@id"]
                    }">(align Glosses)</a>
                    <div class="pull-right col-6">
                        <img src="${b.images[0].resource["@id"]}">
                    </div>
                        ${b.otherContent[0].resources.reduce(
                          (aa, bb) =>
                            (aa += `
                        <span class="line" title="${bb["@id"]}">${
...
DEER.TEMPLATES.glossAssignments = function (obj, options = {}) {
  ...
  return {
    ...
    then: (elem) => {
      UTILS.listFromCollection(options.collection).then((glosses) => {
        elem.innerHTML = glosses.reduce(
          (a, b, i) =>
...
function AttachClickToRemoveHandler(elem) {
  elem.addEventListener("click", (ev) => {
    ...
    document
      .querySelector(
        `.glossBtn[deer-id='${elem.getAttribute("data-badge-uri")}']`
      )
      ?.glossAssignments.delete(elem.closest("line").getAttribute("title"));
  ...
  return {
    html: `
        <div class="page">
            ...
            <div class="col">
                ...
                ${c.otherContent[0].resources.reduce(
                  (aa, bb, i) =>
                    (aa += `
                <line title="${bb["@id"]}" index="${i}">${
                      bb.resource["cnt:chars"].length
                        ? bb.resource["cnt:chars"]
                        : "[ empty line ]"
                    }<i class="gloss-badge tag is-small bd-dark bg-light text-dark"></i></line>
                `),
                  ``
                )}
        </div>
    ...
    then: (elem) => {
      ...
      for (const l of allLines) {
        l.addEventListener("click", selectLine);
      }
      ...
      function selectLine(event) {
        ...
        if (lastClick && event.shiftKey) {
          ...
          const change = lastClick.classList.contains("selected") // change is constant
            ...
            : "remove";
          const lookNext =
            parseInt(lastClick.getAttribute("index")) <
            parseInt(line.getAttribute("index"))
              ? "nextElementSibling"
        ...
        if (lastClick) {
          lastClick.classList.remove("just");
        }
      ...
      for (const b of controls) {
        b.addEventListener("click", (e) => {
          ...
          Array.from(allLines)
            .filter((el) => !el.classList.contains("located"))
            .forEach((l) => {
              l.classList[change]("selected");
              l.classList.remove("just");
            });
        });
      }
      ...
      function connectLinesWithNamedGloss() {
        ...
        let allGlosses = [...glossBtns].map((page) => {
          ...
          return fetch(
            page.dataset.glossPages ? DEER.URLS.OVERWRITE : DEER.URLS.CREATE,
            {
        ...
        Promise.all(allGlosses)
          .then((glosses) => {
            glosses.map((gloss) => {
              document
                .querySelector(`[deer-id='${gloss.new_obj_state.target}']`)
                .setAttribute("data-gloss-pages", gloss.new_obj_state["@id"]);
            });
            const ev = new CustomEvent("Gloss assignment Update");
            UTILS.globalFeedbackBlip(
              ev,
              `Gloss assignment updated successfully.`,
              true
            );
      ...
      function renderGlossDesignations() {
        ...
        fetch(`${DEER.URLS.QUERY}?limit=100&skip=0`, {
          ...
          .catch((err) => {
            ...
            UTILS.globalFeedbackBlip(
              ev,
              `Please reload. This crashed. ${err}`,
              false
            );
          });
      }
        ...
        <div class="page">
            ...
            <div class="col">
                ...
                ${c.otherContent[0].resources.reduce(
                  (aa, bb, i) =>
                    (aa += `
                <line title="${bb["@id"].split("/").pop()}" index="${i}">${
                      bb.resource["cnt:chars"].length
                        ? bb.resource["cnt:chars"]
                        : "[ empty line ]"
                    }<i class="unassign tag is-small bg-light text-dark">⭯</i></line>
                `),
                  ``
                )}
        </div>
    ...
    then: (elem) => {
      ...
      for (const r of unassignmentButtons) {
        r.addEventListener("click", (e) => {
          ...
          const location = Array.from(forLine.classList).filter((val) =>
            classes.includes(val)
          )?.[0];
      ...
      function saveLocations() {
        ...
        allLines.forEach((line) => {
          const location = Array.from(line.classList).filter((val) =>
            classes.includes(val)
          )?.[0];
          if (location) {
        ...
        fetch(`${DEER.URLS.QUERY}?limit=100&skip=0`, {
          ...
          .then((annotations) => {
            ...
            let existingMarginAnnos = {
              ulm:
                annotations.filter(
                  (anno) => anno.body.location === "ulm"
                )?.[0]?.["@id"] ?? false,
              urm:
                annotations.filter(
                  (anno) => anno.body.location === "urm"
                )?.[0]?.["@id"] ?? false,
              um:
                annotations.filter(
                  (anno) => anno.body.location === "um"
                )?.[0]?.["@id"] ?? false,
              llm:
                annotations.filter(
                  (anno) => anno.body.location === "llm"
                )?.[0]?.["@id"] ?? false,
              lm:
                annotations.filter(
                  (anno) => anno.body.location === "lm"
                )?.[0]?.["@id"] ?? false,
              lrm:
                annotations.filter(
                  (anno) => anno.body.location === "lrm"
                )?.[0]?.["@id"] ?? false,
              ti:
                annotations.filter(
                  (anno) => anno.body.location === "ti"
                )?.[0]?.["@id"] ?? false,
              li:
                annotations.filter(
                  (anno) => anno.body.location === "li"
                )?.[0]?.["@id"] ?? false,
            };
            ...
            for (let margin in existingMarginAnnos) {
              if (existingMarginAnnos[margin]) {
                drawAssignment(
                  margin,
                  annotations.filter(
                    (anno) => anno["@id"] === existingMarginAnnos[margin]
                  )[0]
                );
                pageElement.setAttribute(
                  `data-${margin}`,
                  existingMarginAnnos[margin]
                );
              } else {
                ...
                fetch(DEER.URLS.CREATE, {
                  ...
                  .then((loc) => {
                    ...
                    UTILS.globalFeedbackBlip(
                      ev,
                      `Marginalia locations loaded`,
                      true
                    );
                    pageElement.setAttribute(
                      `data-${margin}`,
                      loc.new_obj_state["@id"]
                    );
                  })
  ...
  return {
    ...
    then: (elem) => {
      ...
      function saveLocations() {
        ...
        const locationAnnotationId = document
          .querySelector("div.page")
          .getAttribute("data-marginalia");
      ...
      function highlightLocations() {
        ...
        function drawAssignment(glossLines) {
          for (const line in glossLines) {
            ...
            const locatedClass = glossLines[line]
              ? `located ${glossLines[line]}`
              : "";
            el.className = locatedClass;
          }
        }
...
DEER.TEMPLATES.managedlist = function (obj, options = {}) {
  try {
    ...
    // UI Components for Filtering and Feedback
    let tmpl = `
        <style>
            .cachedNotice{
                margin-top: -1em;
    ...
    const type = obj.name.includes("Named-Glosses")
      ? "named-gloss"
      : "manuscript";

    // Caching
    const cachedManagedListEntities = localStorage.getItem("expandedEntities")
      ? new Map(
          Object.entries(JSON.parse(localStorage.getItem("expandedEntities")))
        )
      : new Map();
    let numloaded = 0;
      ...
      obj[options.list].forEach((val, index) => {
        ...
        tmpl += `<li>
                ...
                <a href="${options.link}${val["@id"]}">
                    <deer-view deer-id="${val["@id"]}" deer-template="label">${
          index + 1
        }</deer-view>
                </a>
                ${removeBtn}
    ...
    return {
      ...
      then: (elem) => {
        ...
        filter.addEventListener("input", (ev) => {
          ...
          debounce(filterGlosses(val));
        });

        function filterGlosses(query = "") {
        ...
        fetch(elem.getAttribute("deer-listing"))
          ...
          .then((list) => {
            ...
            list.itemListElement?.forEach((item) =>
              elem.listCache.add(item["@id"])
            );
            for (const a of document.querySelectorAll(".togglePublic")) {
              const include = elem.listCache.has(a.getAttribute("href"))
                ? "add"
                : "remove";
          ...
          .then(() => {
            document.querySelectorAll(".removeCollectionItem").forEach((el) =>
              el.addEventListener("click", (ev) => {
                ev.preventDefault();
                ev.stopPropagation();
            ...
            document.querySelectorAll(".togglePublic").forEach((a) =>
        ...
        function overwriteList() {
          ...
          elem.listCache.forEach((uri) => {
            mss.push({
              label: document
                .querySelector(`deer-view[deer-id='${uri}']`)
                .textContent.trim(),
              "@id": uri,
            });
          });
        ...
        async function removeFromCollectionAndDelete(id, type) {
          ...
          if (type === "manuscript") {
            ...
            const allGlossIds = await UTILS.getPagedQuery(
              100,
              0,
              allGlossesOfManuscriptQueryObj
            )
            ...
            const allGlosses = allGlossIds.map((glossUri) => {
              return fetch(config.URLS.DELETE, {
                method: "DELETE",
                body: JSON.stringify({
                  "@id": glossUri.replace(/^https?:/, "https:"),
                }),
                headers: {
                  "Content-Type": "application/json; charset=utf-8",
                  Authorization: `Bearer ${window.GOG_USER.authorization}`,
          ...
          const allAnnotationIds = await UTILS.getPagedQuery(
            100,
            0,
            allAnnotationsTargetingEntityQueryObj
          )
          ...
          const allAnnotations = allAnnotationIds.map((annoUri) => {
            return fetch(config.URLS.DELETE, {
              method: "DELETE",
              body: JSON.stringify({
                "@id": annoUri.replace(/^https?:/, "https:"),
              }),
              headers: {
                "Content-Type": "application/json; charset=utf-8",
                Authorization: `Bearer ${window.GOG_USER.authorization}`,
              ...
              .catch((err) => {
                console.warn(
                  `There was an issue removing an Annotation: ${annoUri}`
                );
                console.log(err);
              });
          });
          ...
          Promise.all(allAnnotations)
            ...
            .catch((err) => {
              // OK they may be orphaned.  We will continue on towards deleting the entity.
              console.warn(
                "There was an issue removing connected Annotations."
              );
              console.log(err);
            });
          ...
          fetch(config.URLS.DELETE, {
            ...
            .catch((err) => {
              alert(
                `There was an issue removing the ${thing} with URI ${id}.  This item may still appear in collections.`
              );
              console.log(err);
            });
        }
  ...
  for (let key in obj) {
    ...
    try {
      if ((value.image ?? value.trim()).length > 0) {
        list +=
          label === "depiction"
            ? `<img title="${label}" src="${value.image ?? value}" ${
                DEER.SOURCE
              }="${UTILS.getValue(obj[key].source, "citationSource")}">`
            : `<dt deer-source="${UTILS.getValue(
                obj[key].source,
                "citationSource"
              )}">${label}</dt><dd>${value.image ?? value}</dd>`;
      }
    } catch (err) {
      ...
      if (Array.isArray(value)) {
        value.forEach((v, index) => {
          let name = UTILS.getLabel(
            v,
            v.type ?? v["@type"] ?? label + "" + index
          );
          list += v["@id"]
            ? `<dd><a href="#${v["@id"]}">${name}</a></dd>`
            : `<dd ${DEER.SOURCE}="${UTILS.getValue(
                v.source,
                "citationSource"
              )}">${UTILS.getValue(v)}</dd>`;
        });
        ...
        if (typeof value === "string") {
          value = {
            ...
            source: {
              ...
              comment:
                "Learn about the assembler for this object at https://github.com/CenterForDigitalHumanities/deer",
            },
          };
        }
        ...
        list += value["@id"]
          ? `<dd ${DEER.SOURCE}="${UTILS.getValue(
              value.source,
              "citationSource"
            )}"><a href="${options.link ?? ""}#${value["@id"]}">${v}</a></dd>`
          : `<dd ${DEER.SOURCE}="${UTILS.getValue(
              value,
              "citationSource"
            )}">${v}</dd>`;
...
DEER.TEMPLATES.person = function (obj, options = {}) {
  try {
    let tmpl = `<h2>${UTILS.getLabel(obj)}</h2>`;
    let dob =
      DEER.TEMPLATES.prop(obj, { key: "birthDate", label: "Birth Date" }) ?? ``;
    let dod =
      DEER.TEMPLATES.prop(obj, { key: "deathDate", label: "Death Date" }) ?? ``;
    let famName =
      (obj.familyName && UTILS.getValue(obj.familyName)) ?? "[ unknown ]";
    let givenName = (obj.givenName && UTILS.getValue(obj.givenName)) ?? "";
    tmpl +=
      obj.familyName ?? obj.givenName
        ? `<div>Name: ${famName}, ${givenName}</div>`
        : ``;
    tmpl += dob + dod;
    ...
    then: (elem) => {
      ...
      fetch(`${DEER.URLS.QUERY}?limit=100&skip=0`, {
        ...
        .then((pages) =>
          pages.reduce(
            (a, b) =>
              (b += `<deer-view deer-id="${
                a["@id"] ?? a.id
              }" deer-template="gloss">range</deer-view>`),
            ``
          )
        )
        ...
        .then(() =>
          setTimeout(
            UTILS.broadcast(undefined, DEER.EVENTS.NEW_VIEW, elem, {
              set: elem.querySelectorAll("[deer-template]"),
            }),
            0
          )
        );
  ...
  constructor(elem, deer = {}) {
    ...
    try {
      if (!(this.id ?? this.collection)) {
        ...
        if (this.id) {
          this.id = !this.id.includes("localhost")
            ? this.id.replace(/^https?:/, "https:")
            : this.id; // avoid mixed content
          limiter(() =>
            fetch(this.id)
              .then((response) => response.json())
              .then((obj) => RENDER.element(this.elem, obj))
              .catch((err) => err)
          );
            ...
            return fetch(`${q}?limit=${lim}&skip=${it}`, {
              ...
              .then((list) => {
                listObj.itemListElement = listObj.itemListElement.concat(
                  list.map((anno) => ({
                    "@id": anno.target ?? anno["@id"] ?? anno.id,
                  }))
                );
                this.elem.setAttribute(DEER.LIST, "itemListElement");
                try {
                  listObj["@type"] =
                    list[0]["@type"] ?? list[0].type ?? "ItemList";
                } catch (err) {}
    ...
    if (listensTo) {
      elem.addEventListener(DEER.EVENTS.CLICKED, (e) => {
        try {
          if (
            e.detail.target
              .closest(DEER.VIEW + "," + DEER.FORM)
              .getAttribute("id") === listensTo
          )
            elem.setAttribute(
              DEER.ID,
              e.detail.target.closest("[" + DEER.ID + "]").getAttribute(DEER.ID)
            );
        } catch (err) {}
      });
      try {
        window[listensTo].addEventListener("click", (e) =>
          UTILS.broadcast(e, DEER.EVENTS.CLICKED, elem)
        );
      } catch (err) {
        console.error(
          "There is no HTML element with id " +
            listensTo +
...
export function initializeDeerViews(config) {
  return new Promise((res) => {
    ...
    document.addEventListener(DEER.EVENTS.NEW_VIEW, (e) =>
      Array.from(e.detail.set).forEach((elem) => new DeerRender(elem, config))
    );
    /**
...
function userHasRole(roles) {
  ...
  return Boolean(
    window.GOG_USER?.["http://rerum.io/user_roles"]?.roles.filter((r) =>
      roles.includes(r)
    ).length
  );
}
